...
import { getAllRepairs } from '../utils/db';
  ...
  useEffect(() => {
    const fetchRepairData = async () => {
      try {
        ...
        const stats = allRepairs.reduce(
          (acc, curr) => {
            acc.totalInRepair++;
            acc.pending++;
            acc.totalValue += Number(curr.estimatedCost || 2300);
            return acc;
          },
        ...
        setMetrics(stats);
        setRepairs(allRepairs.reverse()); // Set the latest first
      } catch (error) {
                    ...
                    <td className="px-6 py-6 flex items-center gap-4">
                      <div className="w-10 h-10 bg-[#fafafa] rounded-xl flex items-center justify-center overflow-hidden border border-gray-50">
                        {repair.images && repair.images[0] ? (
                          <img 
                            src={URL.createObjectURL(repair.images[0])} 
