...
const CompactRepairIntake = () => {
  ...
  const uploadToCloudinary = async (file: File) => {
    ...
    data.append('upload_preset', 'PRODUCTS'); // Replace with your unsigned Cloudinary preset if needed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('repairs').insert([repairToSave]);
      if (error) {
        throw error;
      }

      alert("Slip logged successfully to database.");
      ...
      alert(error.message || "Error: Could not log repair. Please check your connection.");
    } finally {
      setIsSaving(false);
    }
  };
