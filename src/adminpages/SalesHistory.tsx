...
const Modal = ({ sale, onClose }: { sale: SaleRecord; onClose: () => void }) => {
  ...
  const handleDownload = () => {
    ...
    // Cloudinary Force Download Fix
    let downloadUrl = sale.deliverynote;
    ...
    link.setAttribute('download', `Note_${sale.buyername.replace(/\s+/g, '_')}.pdf`);
    // target="_blank" handles cases where the download is blocked by pop-up blockers
